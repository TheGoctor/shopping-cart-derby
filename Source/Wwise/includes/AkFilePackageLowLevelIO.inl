    ...
    )
{
	...
	if ( bAligned )
	{
		...
		pBufferForHeader += ( CAkFileHelpers::s_uRequiredBlockSize - (AkUIntPtr)pBufferForHeader % CAkFileHelpers::s_uRequiredBlockSize );
...
{
	const size_t numChars = AKPLATFORM::OsStrLen( T_LLIOHOOK_FILELOC::m_szLangSpecificDirName );
