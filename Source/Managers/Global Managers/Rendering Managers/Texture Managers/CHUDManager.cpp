...
void CHUDManager::Init(void)
{
	...
	m_pKey1Comp = m_pTM->CreateBitmapFontComp(pTimerObj, "1", font, 400, 10, 1.5f, D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f), false);
	m_pKey2Comp = m_pTM->CreateBitmapFontComp(pTimerObj, "2", font, 400, 10, 1.5f, D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f), false);
...
void CHUDManager::InitShoppingList(void)
{
	...
	m_tListIconData[7].m_dwColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
...
void CHUDManager::InitInventory(void)
{
	...
	tInventory.m_dwColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
	...
	tbutton1.m_dwColor = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
...
void CHUDManager::Update(void)
{
	...
	while(nodeIter != m_lRadarNodes.end())
	{
		...
		{
			list<TPlayerRadarNodeInfo, CAllocator<TPlayerRadarNodeInfo>>::iterator nodeIterToFind;
...
void CHUDManager::GoalItemLostCallback(IEvent* pEvent, IComponent* /*pComp*/)
{
	...
	if(nDropper == 0)
	{
		...
		list<TShoppingListItem, CAllocator<TShoppingListItem>>::iterator goalItemIter2;
		...
		for(int i=0; i<4; i++)
		{
			...
			if(GetInstance()->m_tPlayerInfo[i].nID == nDropper)
			{
				...
				if(pPlayerObj)
				{
					...
					list<TPlayerRadarNodeInfo, CAllocator<TPlayerRadarNodeInfo>>::iterator infoNodeIter;
...
void CHUDManager::InitCheckoutLocation(IEvent* pEvent, IComponent* /*pComp*/)
{
	...
	list<TRadarNode, CAllocator<TRadarNode>>::iterator iter;
