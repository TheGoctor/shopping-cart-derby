...
// Constructor
CStealEffect::CStealEffect(void) : m_pItemComp(NULL), m_pBlastComp(NULL),
								   m_fScaleCounter(0.0f), m_fCounter(0.0f),
								   m_bActive(false), m_bScaleUp(true)
{
}
...
TSpriteData CStealEffect::GetBlastInitData(void)
{
	...
	int texID = pTM->LoadTexture("Resource\\Textures\\Donut Steal Sprites\\FFP_2D_Shine2_FIN.png");
	TSpriteData tData;
...
void CStealEffect::CreateSpriteComps(void)
{
	...
	m_pBlastComp = CTextureManager::GetInstance()->CreateSpriteComp(pBlastObj, GetBlastInitData(), false);
}
