										   ...
										   TAABB& outAABB, TAABB& outLocalAABB)
{
	...
	for(unsigned int i = 0; i < currAABBIndex; ++i)
	{
		...
		{
			//float fDiff = m_pAABBBuffer[i].aabb.cBoxMax.z - m_pAABBBuffer[i].aabb.cBoxMin.z;

			outAABB.cBoxMax = m_pAABBBuffer[i].aabb.cBoxMax + vectoroffset /* + D3DXVECTOR3(0, 0, fDiff)*/;
			outAABB.cBoxMin = m_pAABBBuffer[i].aabb.cBoxMin + vectoroffset /* + D3DXVECTOR3(0, 0, fDiff)*/;
			outLocalAABB.cBoxMax = m_pAABBBuffer[i].aabb.cBoxMax/* + D3DXVECTOR3(0, 0, fDiff)*/;
			outLocalAABB.cBoxMin = m_pAABBBuffer[i].aabb.cBoxMin/* + D3DXVECTOR3(0, 0, fDiff)*/;
...
DXMesh* ModelManager::CreateCubeFromAABB(TAABB aabb)
{
	...
	DXMesh *rendercube = MMNEW(DXMesh) DXMesh();

	// Create Verts
		
		// Position
		VERTEX_POSCOLOR *verts = MMNEW(VERTEX_POSCOLOR[8]) VERTEX_POSCOLOR();
