	...
	)
{
	...
	{
		assert( !"CAkDefaultIOHookDeferred I/O hook only works with AK_SCHEDULER_DEFERRED_LINED_UP devices" );
		return AK_Fail;
	}
	...
	if ( m_deviceID != AK_INVALID_DEVICE_ID )
	{
		...
		{
			assert( !"Failed creating pool for asynchronous device" );
			return AK_Fail;
		}
...
)
{
	...
	{
		eResult = AK_Success;
		assert( dwNumberOfBytesTransfered >= pXferInfo->uRequestedSize && dwNumberOfBytesTransfered <= pXferInfo->uBufferSize );
	}
	...
	)
{
	...
	assert( io_transferInfo.uFilePosition % GetBlockSize( in_fileDesc ) == 0
			|| !"Requested file position for I/O transfer is inconsistent with block size" );
