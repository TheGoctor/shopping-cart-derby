...
model_manager::model_manager() { currIndex = 0; }

void model_manager::Init(void) {
  // Register for Events
...
// Shutdown
void model_manager::ShutdownCallback(IEvent*, IComponent*) {
  GetInstance()->Shutdown();
}
void model_manager::Shutdown(void) {
  // Meshs
  map<unsigned int,
      DXMesh*,
      less<unsigned int>,
      CRenderAllocator<pair<unsigned int, DXMesh*>>>::iterator meshIter =
      m_cMeshTemplates.begin();
  ...
  map<unsigned int,
      TMeshVertexInfo*,
      less<unsigned int>,
      CRenderAllocator<pair<unsigned int, TMeshVertexInfo*>>>::iterator
      vertIter = m_cVertexTemplates.begin();
  ...
  map<unsigned int,
      CAnimation*,
      less<unsigned int>,
      CRenderAllocator<pair<unsigned int, CAnimation*>>>::iterator animIter =
...
int model_manager::LoadModel(char* szFileName) {
  // read String Size
  // read Name of Model
  // read Loop size
  // read Vertices
  ...
  if (fin.is_open()) {
    ...
    bool bStoringVerts =
        (strcmp(
             m_pMeshBuffer[currIndex].m_szName, "FFP_3D_CheckoutCollision_FIN")
         == 0);
    if (bStoringVerts) {
    ...
    this->m_pMeshBuffer[currIndex].CreateVertexBuffer(
        verts,
        num,
    ...
    if (bStoringVerts) {
      m_cVertexTemplates.insert(make_pair(
          CIDGen::GetInstance()->GetID(m_pMeshBuffer[currIndex].m_szName),
          pVertInfo));
    }
...
int model_manager::LoadModelWithBones(char* szFileName) {
  // read String Size
  // read Name of Model
  // read Loop size
  // read Vertices
...
int model_manager::LoadAABB(char* szMeshName) {
  unsigned int num = 0;
  char buffer[128];
...
// TAABB model_manager::CreateAABB(D3DXVECTOR3* pPosInfo)
//{
//
//}

int model_manager::GetMeshIndexByName(
...
bool model_manager::GetAABBByNameWithOffset(
    char* szMeshName,
    D3DXVECTOR3 trans,
    scd::transform rot,
    TAABB& outAABB,
  ...
  for (unsigned int i = 0; i < currAABBIndex; ++i) {
    if (strcmp(search.c_str(), m_pAABBBuffer[i].m_szName) == 0) {
      ...
      D3DXVECTOR3 newmax(
          max(rotmin.x, rotmax.x),
          max(rotmin.y, rotmax.y),
          max(rotmin.z, rotmax.z));

      D3DXVECTOR3 newmin(
          min(rotmin.x, rotmax.x),
...
DXMesh* model_manager::CreateCubeFromAABB(TAABB aabb) {
  // Get Singletons
  Direct3DManager* pD3DMan = Direct3DManager::GetInstance();
  ...
  verts[1].position =
      D3DXVECTOR3(aabb.cBoxMin.x, aabb.cBoxMax.y, aabb.cBoxMin.z);
  verts[2].position =
      D3DXVECTOR3(aabb.cBoxMax.x, aabb.cBoxMax.y, aabb.cBoxMin.z);
  verts[3].position =
      D3DXVECTOR3(aabb.cBoxMax.x, aabb.cBoxMin.y, aabb.cBoxMin.z);
  verts[4].position =
      D3DXVECTOR3(aabb.cBoxMin.x, aabb.cBoxMin.y, aabb.cBoxMax.z);
  verts[5].position =
      D3DXVECTOR3(aabb.cBoxMin.x, aabb.cBoxMax.y, aabb.cBoxMax.z);
  verts[6].position = aabb.cBoxMax;
  verts[7].position =
      D3DXVECTOR3(aabb.cBoxMax.x, aabb.cBoxMin.y, aabb.cBoxMax.z);
  ...
  // Create Vert Buffer
  rendercube->CreateVertexBuffer(
      verts,
      8,
      pD3DMan->GetVertColorDecl(),
  ...
  DWORD nCudeIdxs[nNumCubeIdxs] = {// Front
                                   0,
                                   1,
                                   2,
                                   0,
...
int model_manager::LoadAnimFile(char* szFileName) {
  // Get Singletons
  CIDGen* pIDGen = CIDGen::GetInstance();
  ...
  if (fin.is_open()) {
    ...
    // Add to Map
    m_cAnimationTemplates.insert(
        make_pair(pIDGen->GetID(szAnimName), pAnimData));
...
DXMesh* model_manager::GetAnimMesh(string szMeshName) {
  // LoadModelWithBones
  map<unsigned int,
      DXMesh*,
      less<unsigned int>,
      CRenderAllocator<pair<unsigned int, DXMesh*>>>::iterator pMeshIter;
...
CAnimation* model_manager::GetAnim(string szAnimName) {
  map<unsigned int,
      CAnimation*,
      less<unsigned int>,
      CRenderAllocator<pair<unsigned int, CAnimation*>>>::iterator pAnimIter;
...
CAnimation* model_manager::GetAnim(unsigned int nAnimID) {
  map<unsigned int,
      CAnimation*,
      less<unsigned int>,
      CRenderAllocator<pair<unsigned int, CAnimation*>>>::iterator pAnimIter;
...
DXMesh* model_manager::GetCloneMesh(string szMeshName) {
  // Find Original Mesh
  ...
  // Find Original Verts
  TMeshVertexInfo* pOriginalVerts =
      (m_cVertexTemplates.find(CIDGen::GetInstance()->GetID(szMeshName)))
          ->second;
  ...
  for (int bone = 0; bone < nNumBones; ++bone) {
    ...
    for (int nChar = 0; nChar < nStrLen + 1; ++nChar) {
      tBone.m_szBoneName[nChar] =
          pOriginalMesh->GetBones()[bone].m_szBoneName[nChar];
    }
    ...
    for (int child = 0; child < tBone.m_nNumChildren; ++child) {
      tBone.m_cChildrenIdxs[child] =
          pOriginalMesh->GetBones()[bone].m_cChildrenIdxs[child];
    }
...
TMeshVertexInfo* model_manager::GetCloneVerts(string szMeshName) {
  // Create a Clone
