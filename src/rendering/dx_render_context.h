...
struct RenNodeCmp
{
	...
	{
		// Get Camera Pos
		scd::transform camMap = CCameraManager::GetInstance()->GetCam()->GetViewMatrix(); 
...
public:
	...
	{
		m_pShaderEffect->SetValue("diffuseLightColor", &DifColor, sizeof(scd::vector4));
	}

	inline void SetDiffuseMtrl(const scd::vector4 &DifMtrl)
	{
		m_pShaderEffect->SetValue("diffuseMtrlColor", &DifMtrl, sizeof(scd::vector4));
	}
