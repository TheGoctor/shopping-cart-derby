	...
	if (m_dt > 0.1f)
	{
		...
		if (m_pGPSplash->IsActive())
		{
			if (m_fSeconds < 1)
			{
				...
				m_fAlphaValue += 0.3f;
				data.m_dwColor = scd::vector4(1.0f, 1.0f, 1.0f, m_fAlphaValue);
				m_dt = 0.0f;
		...
		if (m_pTitleScreen->IsActive())
		{
			if (m_fSeconds < 1)
			{
				...
				enter.m_dwColor = scd::vector4(1.0f, 1.0f, 1.0f, m_fAlphaValue);
			...
			else if (m_fSeconds > 1.1 && m_fSeconds < 1.2f )
			{
				...
				TSpriteData enter = m_pPressEnter->GetSpriteData();
				enter.m_dwColor = scd::vector4(1.0f, 1.0f, 1.0f, m_fTitleBarAlphValue);
				m_pPressEnter->SetSpriteData(enter);
			...
			else if (m_fSeconds > 1.2f && m_fSeconds < 1.3f)
			{
				...
				TSpriteData enter = m_pPressEnter->GetSpriteData();
				enter.m_dwColor = scd::vector4(1.0f, 1.0f, 0.0f, m_fTitleBarAlphValue);
				m_pPressEnter->SetSpriteData(enter);
