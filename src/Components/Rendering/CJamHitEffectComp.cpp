...
void CJamHitEffectComp::Init(void)
{
	...
	// Get Emitters
	CParticleEmitter* pPE0 = pEffMan->GetCloneEmitter("Jam_Trail.bin", m_pDownEffect);
	CParticleEmitter* pPE1 = pEffMan->GetCloneEmitter("Jam_Hit_Meteor_Down.bin", pJamEffect0);

	CParticleEmitter* pPE2 = pEffMan->GetCloneEmitter("Jam_Hit_Splat.bin", pJamEffect1);
	CParticleEmitter* pPE3 = pEffMan->GetCloneEmitter("Splurch_Popup.bin", pJamEffect2);
	CParticleEmitter* pPE4 = pEffMan->GetCloneEmitter("Jam_Hit_Burst.bin", pJamEffect3);
	
	// Create Render Comp
	CRenderComponent* pRenComp0 = pRenMan->CreateRenderComp(m_pDownEffect,
		(DXMesh*)pPE0, RC_JAM_HIT_EFFECT_BLOB, RF_PARTICLE);
	CRenderComponent* pRenComp1 = pRenMan->CreateRenderComp(pJamEffect0,
		(DXMesh*)pPE1, RC_JAM_USE_EFFECT_JAR, RF_PARTICLE);

	CRenderComponent* pRenComp2 = pRenMan->CreateRenderComp(pJamEffect1,
		(DXMesh*)pPE2, RC_JAM_HIT_EFFECT_BLOB, RF_PARTICLE);
	CRenderComponent* pRenComp3 = pRenMan->CreateRenderComp(pJamEffect2,
		(DXMesh*)pPE3, RC_JAM_HIT_EFFECT_SPLURCH_POPUP, RF_PARTICLE);
	CRenderComponent* pRenComp4 = pRenMan->CreateRenderComp(pJamEffect3,
		(DXMesh*)pPE4, RC_JAM_HIT_EFFECT_BLOB, RF_PARTICLE);

	// Create Effect Comps
	m_pDownVFX = pEffMan->CreateEffectComp(m_pDownEffect, pPE0, pRenComp0,
		EC_TYPE_JAM_HIT_FALL, PE_TYPE_JAM_TRAIL_DOWN);
	m_pDownBVFX = pEffMan->CreateEffectComp(m_pDownEffect, pPE1, pRenComp1,
		EC_TYPE_JAM_HIT_FALL_B, PE_TYPE_JAM_HIT_BLOB);
	m_pDownBVFX->SetOnTarget(EC_TYPE_JAM_HIT_FALL_B, true);

	m_pTargetVFX = pEffMan->CreateEffectComp(m_pParentObj, pPE2, pRenComp2, EC_TYPE_JAM_HIT_POPUP,
		PE_TYPE_JAM_HIT_BLOB);
	m_pTargetVFX->AddEmitter(EC_TYPE_JAM_HIT_POPUP, pPE3, pRenComp3, m_pParentObj->GetTransform(),
		PE_TYPE_JAM_HIT_POPUP);
	m_pTargetVFX->AddEmitter(EC_TYPE_JAM_HIT_POPUP, pPE4, pRenComp4, m_pParentObj->GetTransform(),
		PE_TYPE_JAM_BURST);
...
void CJamHitEffectComp::UpdateCallback(IEvent* pEvent, IComponent* pComp)
{
	...
	if(pJHVFXC->m_fInvDuration > 0.0f)
	{
		...
		pJHVFXC->m_pDownVFX->SetDeadTimer(EC_TYPE_JAM_HIT_FALL, pJHVFXC->m_fInvDuration);
		pJHVFXC->m_pDownBVFX->SetDeadTimer(EC_TYPE_JAM_HIT_FALL_B, pJHVFXC->m_fInvDuration);
		pJHVFXC->m_pTargetVFX->SetDeadTimer(EC_TYPE_JAM_HIT_POPUP, pJHVFXC->m_fInvDuration);	
	}
...
void CJamHitEffectComp::Update(float fDT)
{
	...
	if(pFrame->GetLocalPosition().y <= 0.0f)
	{
		...
		if(m_bDead == false)
		{
			CWwiseSoundManager::GetInstance()->PlayTheSound(ITEM_JAM_HIT, 
				CHUDManager::GetInstance()->GetPlayerCharID(CHUDManager::GetInstance()->GetPlayerNum(m_pParentObj)));
		}
	}
}

// Invulnerable
void CJamHitEffectComp::InvulnerableCallback(IEvent* pEvent, IComponent* pComp)
{
	// Get Data from Event
	CJamHitEffectComp* pJHVFXC = (CJamHitEffectComp*)pComp;
...
// Shutdown Gameplay
void CJamHitEffectComp::ShutdownGameplayCallback(IEvent* pEvent, IComponent* pComp)
{
	// Get Data from Event
	CJamHitEffectComp* pJHVFXC = (CJamHitEffectComp*)pComp;
