...
// Shutdown Gameplay
void CJamUseEffectComp::ShutdownGameplayCallback(
    IEvent* pEvent,
    IComponent* pComp) {
  // Get Data from Event
...
void TJamUseVFX::Init(CObject* pParentObj) {
  ...
  // Create Render Comp
  m_pMeshRenComp = pRM->CreateRenderComp(
      m_pMeshObj, snMeshIdx, RC_JAM, RF_INDEXED_VERT_TEX2);
  ...
  CParticleEmitter* pPE0 =
      pEM->GetCloneEmitter("Jam_Use_Trail.bin", pJamEffect0);

  // Create Render Comp
  CRenderComponent* pRenComp0 = pRM->CreateRenderComp(
      pJamEffect0, (DXMesh*)pPE0, RC_JAM_HIT_EFFECT_BLOB, RF_PARTICLE);

  // Create Effect Comps
  m_pTrailEffect = pEM->CreateEffectComp(
      m_pMeshObj, pPE0, pRenComp0, EC_TYPE_JAM_USE, PE_TYPE_JAM_TRAIL_UP);
...
void TJamUseVFX::Update(float fDT) {
  ...
  // Translate Mesh
  pMeshFrame->TranslateFrame(
      D3DXVECTOR3(0.0f, JUE_MESH_MOVE_Y_RATE, 0.0f) * fDT);
