...
void TFirework::Init(CObject* pParent) {
  ...
  // m_pEmitterObj->GetTransform()->TranslateFrame(D3DXVECTOR3(-1.0f,
  // 0.3f, 1.0f));
...
void TFirework::ResetFirework(void) {
  ...
  m_pEffect->ChangeContext(
      EC_TYPE_FIREWORK_TRAIL, PE_TYPE_FIRE_WORK_TRAIL, eRenderConxtext);
  m_pEffect->ChangeContext(
      EC_TYPE_FIREWORK_BURST, PE_TYPE_FIRE_WORK_BURST, eRenderConxtext);
...
void CVictoryVFXComp::ShutdownCallback(IEvent* pEvent, IComponent* pComp) {
  ...
  for (int firework = 0; firework < MAX_FIREWORKS; ++firework) {
    pVFX->m_tFireworks[firework].m_pEffect->SwitchOnOffEmitters(
        EC_TYPE_FIREWORK_TRAIL, false);
    pVFX->m_tFireworks[firework].m_pEffect->SwitchOnOffEmitters(
        EC_TYPE_FIREWORK_BURST, false);
  }
  // pVFX->m_tFirework.m_pEffect->SwitchOnOffEmitters(EC_TYPE_FIREWORK_TRAIL,
  // false);
  // pVFX->m_tFirework.m_pEffect->SwitchOnOffEmitters(EC_TYPE_FIREWORK_BURST,
  // false);
  // pVFX->m_tFirework2.m_pEffect->SwitchOnOffEmitters(EC_TYPE_FIREWORK_TRAIL,
  // false);
  // pVFX->m_tFirework2.m_pEffect->SwitchOnOffEmitters(EC_TYPE_FIREWORK_BURST,
  // false);
