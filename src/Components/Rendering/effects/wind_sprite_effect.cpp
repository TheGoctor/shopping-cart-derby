...
namespace scd {
// Constructor
CWindSpriteEffect::CWindSpriteEffect(void)
    : m_pWindScreenSprite(NULL)
    , m_fAnimTimer(0.0f)
    , m_nAnimCell(0)
    , m_fDuration(0.0f)
    , m_bActive(false) {}
...
void CWindSpriteEffect::CreateWindSprite(void) {
  // Screen Sprite
  int nTexID = CTextureManager::GetInstance()->LoadTexture(
      "Resource\\Textures\\Boost Sprites\\FFP_2D_wind_FIN.png");
  TSpriteData tData;
  ...
  m_pWindScreenSprite =
      CTextureManager::GetInstance()->CreateSpriteComp(pObj, tData, false);
}
...
// Cell Algo
RECT CWindSpriteEffect::CellAlgo(
    int nID,
    int nNumCols,
    int nCellWidth,
    int nCellHeight) {
