...
void CSlipVFXComp::CreateRightDrip(void) {
  ...
  m_pRightEmitter->GetTransform()->TranslateFrame(
...
void CSlipVFXComp::Slip(TStatusEffectEvent* pcEvent) {
  ...
  if (pcEvent->m_pObject->GetID() == m_pParentObj->GetID()) {
    m_pLeftEffect->ChangeContext(
        EC_TYPE_BANANA_SLIP, PE_TYPE_BANANA_DRIP, RC_DRIP_LEFT);
    m_pRightEffect->ChangeContext(
        EC_TYPE_BANANA_SLIP, PE_TYPE_BANANA_DRIP, RC_DRIP_RIGHT);
...
void CSlipVFXComp::Slow(TStatusEffectEvent* pcEvent) {
  // Did we Slow
  if (pcEvent->m_pObject->GetID()
      == m_pParentObj->GetID()) // && m_fSlowCooldown == 0.0f)
  {
    ...
    m_pLeftEffect->ChangeContext(
        EC_TYPE_BANANA_SLIP, PE_TYPE_BANANA_DRIP, RC_PB_DRIP_LEFT);
    m_pRightEffect->ChangeContext(
        EC_TYPE_BANANA_SLIP, PE_TYPE_BANANA_DRIP, RC_PB_DRIP_RIGHT);
    m_pLeftEffect->SetDeadTimer(EC_TYPE_BANANA_SLIP, 0.0f);
    m_pRightEffect->SetDeadTimer(EC_TYPE_BANANA_SLIP, 0.0f);
