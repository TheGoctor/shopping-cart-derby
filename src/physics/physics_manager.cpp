...
void CCollisionManager::Init()
{
	...
	m_pTurkeyDestroy = scd::objectManager::CreateObject("TurkeyDestroyObject");
	m_pPieDestroy = scd::objectManager::CreateObject("PieDestroyObject");
	m_pBananaDestroy = scd::objectManager::CreateObject("BananaDestroyObject");
	...
	char buffer[64];
	unsigned int num = 0;
	scd::vector3 *pos_buffer;
	scd::vector3 *norm_buffer;
	...
	if(fin.is_open())
	{
		...
		pos_buffer = new scd::vector3[num];
		norm_buffer = new scd::vector3[num];
		...
		for(unsigned int i = 0; i < num; ++i)
		{
			...
			scd::vector3 e0 = m_pWallTris[i].tVert2 - m_pWallTris[i].tVert0;
			scd::vector3 e1 = m_pWallTris[i].tVert1 - m_pWallTris[i].tVert0;
...
int CCollisionManager::CreateCollideableComponent(lua_State* pLua)
{
	...
	case BSPHERE:
		{
			scd::object* pObject = (scd::object*)lua_topointer(pLua, -9);
	...
	case BAABB:
		{
			scd::object* pObject = (scd::object*)lua_topointer(pLua, -11);
...
scd::vector3 CCollisionManager::GetCollisionPtSpheres(CCollideable* obj1, CCollideable* obj2)
{
	scd::vector3 ColPt, p1, p2;
	ColPt = p1 = p2 = scd::vector3(0.0f, 0.0f, 0.0f);
	p1 = obj1->GetSphere().cPosition;
	p2 = obj2->GetSphere().cPosition;
	float fDist = 0.0f;
	//get the vector between the two spheres
	scd::vector3 vecbetween = p2 - p1;
...
void CCollisionManager::CondenseCollisionBoxes(void)
{
	...
	for (unsigned i = 0; i < combinedboxes.size(); ++i)
	{
		...
		szName+=(char)i;
		scd::object* pColObj = scd::objectManager::GetInstance()->CreateObject(szName);
...
void CCollisionManager::Shutdown(IEvent* /* pEvent*/, scd::base_component* /* pComponen*/)
{
	std::map<unsigned int, CCollideable*, less<unsigned int>, scd::allocator<pair<
	...
	scd::objectManager::GetInstance()->DestroyObject(GetInstance()->GetTurkeyDestroyObject());
	scd::objectManager::GetInstance()->DestroyObject(GetInstance()->GetPieDestroyObject());
	scd::objectManager::GetInstance()->DestroyObject(GetInstance()->GetBananaDestroyObject());
}

void CCollisionManager::Update(IEvent* pEvent, scd::base_component* pComponent)
{
												   ...
												   scd::vector3* vReflect, bool bReact)
{
	...
	float fDist = 0.0f;
	scd::vector3 v1to2 = s2.cPosition-s1.cPosition;
	...
	if(fDist <= fRadii)
	{
		...
		scd::vector3 v2to1 = s1.cPosition-s2.cPosition;
												 ...
												 scd::vector3* vReflect, bool bReact)
{
	...
	if(fDist <= obj1->GetSphere().fRadius)
	{
		...
		scd::vector3 boxNorm = GetAABBNormal(vclosest, obj2->GetAABB());
											  ...
											  scd::vector3* /*tPosition*/, bool bReact)
{
	...
	bB.tE[1] = D3DXVec3Length( &(scd::vector3(0.0f, bB.cCenterPoint.y, 0.0f) - scd::vector3(0.0f, obj2->GetAABB().cBoxMax.y, 0.0f)) );
	bB.tE[1] = D3DXVec3Length( &(scd::vector3(0.0f, 0.0f, bB.cCenterPoint.z) - scd::vector3(0.0f, 0.0f, obj2->GetAABB().cBoxMax.z)) );
	...
	scd::vector3 vT = bB.cCenterPoint - bA.cCenterPoint;
	//bring the translation into bA's coordinate frame
	vT = scd::vector3( D3DXVec3Dot(&vT, &bA.tU[0]),
										  ...
										  scd::vector3 tTestPt, scd::vector3& tClosest)
{
	...
	scd::vector3 vVec = tTestPt - tBox.cCenterPoint;
												...
												scd::vector3* /*tPosition*/, bool bReact)
{
	...
	scd::vector3 tV = tClosest - obj1->GetSphere().cPosition;
...
bool CCollisionManager::LineToSphereIntersection(TLine tLine, TSphere tSphere, scd::vector3& tColPoint)
{
	//make a vector for the line, and for the linestart to the sphere center
	scd::vector3 vL = tLine.cLineEnd - tLine.cLineStart;
	//get the line direction normal
	scd::vector3 vLN;
	D3DXVec3Normalize(&vLN, &vL);
	scd::vector3 vV = tSphere.cPosition - tLine.cLineStart;
	//dot them together
	float fD = D3DXVec3Dot(&vLN, &vV);
	//scale the normal of the line
	scd::vector3 vNScaled = vLN * fD;
	//determine the closest point
	scd::vector3 closestpt = tLine.cLineStart + vNScaled;	//closest point
	scd::vector3 closesttocenter = tSphere.cPosition = closestpt;
...
bool CCollisionManager::LineToTriangle(TLine tLine, TTriangle triangle, scd::vector3 & tColpoint)
{
	bool hasCollided = false;
	tColpoint = tLine.cLineEnd;
	//d0 = dotprod(N, vstart) ||  d1 = dotprod(N, triangvert[0])  || d2 = d0-d1
	float D0, D1, D2, D3, DF;
	scd::vector3 DL, ColPt;
	...
	//check start
	scd::vector3 edge0, edge1, edge2;
	scd::vector3 Norm0, Norm1, Norm2;
	scd::vector3 VectoPoint;
	float fDtoPoint;
...
bool CCollisionManager::PointInTriangle(TTriangle tTri, scd::vector3 tTestPt)
{
	scd::vector3 Edge0, Edge1, Edge2, Norm0, Norm1, Norm2;
...
float ClosestPtSegmentSegment(scd::vector3 p1, scd::vector3 q1, scd::vector3 p2, scd::vector3 q2,
							  float &s, float &t, scd::vector3 &c1, scd::vector3 &c2)
{
	scd::vector3 d1 = q1 - p1;	// Direction vector of segment 1
	scd::vector3 d2 = q2 - p2;	// Direction vector of segment 2
	scd::vector3 r = p1 - p2;
...
scd::vector3 CCollisionManager::ClosestPointOnLine(TLine line, scd::vector3 testpt)
{
	scd::vector3 closest;
	scd::vector3 linevec, tV;
	float fDot = 0.0f;
...
bool CCollisionManager::RayToLineIntersection(TRay tRay, TLine tSegment)
{
	...
	scd::vector3 temp = tRay.cRayStart + (tRay.cRayNormal * 10.0f);
...
void CCollisionManager::ClosestPointToAABB(TAABB tBox, scd::vector3 tTestPt, scd::vector3& tClosest)
{
	//loop through the axes
	for (unsigned iaxis = 0; iaxis < 3; ++iaxis)
	{
...
scd::vector3 CCollisionManager::GetAABBNormal(scd::vector3 colPt, TAABB colBox)
{
	//checks which axis of the collision point matches an axis on the AABB
	scd::vector3 vNorm;
	scd::vector3 vCenter = (colBox.cBoxMax + colBox.cBoxMin)/2.0f;
	scd::vector3 vRelativeSide = scd::vector3(0.0f, vCenter.y, 0.0f);
...
scd::vector3 CCollisionManager::GetSphereNormal(scd::vector3 colPt, TSphere colSphere)
{
...
scd::vector3 CCollisionManager::GetReflectedVector(scd::vector3 vDir, scd::vector3 vNorm)
{
	scd::vector3 vReflection;
...
scd::vector3 CCollisionManager::GetCloserPt(scd::vector3 tPt1, scd::vector3 tPt2, scd::vector3 tTestpt)
{
	...
	scd::vector3 v1, v2;
	v1 = scd::vector3( tTestpt - tPt1 );
	v2 = scd::vector3( tTestpt - tPt2 );
	...
	case OBJPLAYER:
		{
			...
			case OBJPLAYER:
				{//player to player
					if(OBBToOBBIntersection(obj1, obj2, m_pReflect, true))
					{
						...
						SendRamEvent("CartRam", (scd::base_component*)GetInstance(), obj1->GetParent(), obj2->GetParent());
				...
				{//player to held item
					if(SphereToOBBIntersection(obj2, obj1, m_pReflect, false))
					{
						SendHeldItemCollectedEvent("HeldItemCollision",
							(scd::base_component*)GetInstance(), obj2->GetParent(), obj1->GetParent(), PRIORITY_IMMEDIATE);
						return true;
					}
				}
				break;
			...
			case OBJSHELF:
				{//player to shelf
					scd::vector3 velociraptor = obj1->m_tLocalAxes[2];
													   ...
													   scd::vector3 tVelocity, scd::vector3* /*vReflect*/, bool /*bReact*/)
{
	...
	TAABB box;
	scd::vector3 currReflect, intReflect, nextReflect;
...
scd::vector3 CCollisionManager::ClosestPtPointTriangle(scd::vector3 p, scd::vector3 a, scd::vector3 b, scd::vector3 c)
{
	...
	if (d1 <= 0.0f && d2 <= 0.0f) return scd::vector3(0.0f, 0.0f, 0.0f); //b; // barycentric coordinates (1,0,0)

	// Check if P in vertex region outside B
	scd::vector3 bp = p - b;
	float d3 = D3DXVec3Dot(&ab, &bp);
	float d4 = D3DXVec3Dot(&ac, &bp);
	if(d3 >= 0.0f && d4 <= d3) return scd::vector3(0.0f, 0.0f, 0.0f); //b; // barycentric coordinates (0,1,0)
	...
	{
		//float v = d1 / (d1 - d3);
		return scd::vector3(0.0f, 0.0f, 0.0f); //a + v * ab; // barycentric coordinates (1-v, v, 0)
	}
	...
	if(d6 >= 0.0f && d5 <= d6) return scd::vector3(0.0f, 0.0f, 0.0f); //c; // barycentric coordinates (0,0,1)
	...
	{
		//float w = d2 / (d2 - d6);
		return scd::vector3(0.0f, 0.0f, 0.0f); //a + w * ac; // barycentric coordinates (1-w,0,w)
	}
	...
	if(va <= 0.0f && (d4 - d3) >= 0.0f && (d5 - d6) >= 0.0f)
	{
		...
		return scd::vector3(0.0f, 0.0f, 0.0f); //b + w * (c - b); // barycentric coordinates (0,1-w,w)
	}
...
int CCollisionManager::TestObjAgainstWall(CCollideable* obj1)
{
	...
	scd::vector3 mypos = s.cPosition + scd::vector3(0.0f, 0.1f, 0.0f);

	for(int i = 0; i < m_nTriCount; ++i)
	{
		scd::vector3 p = ClosestPtPointTriangle(mypos, m_pWallTris[i].tVert0, m_pWallTris[i].tVert1,
			m_pWallTris[i].tVert2);

		//scd::vector3 testNorm = (s.cPosition - p);
...
int CCollisionManager::TestObjAgainstWall(scd::vector3 &cameraFrame, scd::vector3 &vFwdVec)
{

	TSphere s;
	s.cPosition = cameraFrame;
	s.fRadius = 0.5f;
	scd::vector3 mypos = s.cPosition;
