 ...
 *
 * @brief Encapsulates collision detection and reaction of all collideable
 * components
 */
#pragma once
...
constexpr bool same_sign(uint32_t a, uint32_t b) {
...
private:
  ...
  //////////////////////////////////////////////////////////////////////////
  scd::map<unsigned int, collider*> _static_objects;
  scd::map<unsigned int, collider*> _dynamic_objects;
  scd::map<unsigned int, collider*> _separated_boxes;

  //////////////////////////////////////////////////////////////////////////
  // reflect vector and collision point, for storing the value after it gets set
  // in 		one of the collision functions
  //////////////////////////////////////////////////////////////////////////
  scd::vector3* _reflect;
  scd::vector3* _collision_point;
  ...
  //////////////////////////////////////////////////////////////////////////
  scd::object* _turkey_destroy;
  scd::object* _pie_destroy;
  scd::object* _banana_destroy;

  //////////////////////////////////////////////////////////////////////////
  // wall collision volume indices and triangles
  //////////////////////////////////////////////////////////////////////////
  scd::vector<TTriangle*> _wall_triangles;

  event_manager* _event_manager;

  physics_manager();
  physics_manager(const physics_manager&);
  physics_manager& operator=(const physics_manager&);
...
public:
  ...
  //////////////////////////////////////////////////////////////////////////
  // shutdown - clears all the memory used by the collision manager and shuts it
  // down
  //////////////////////////////////////////////////////////////////////////
  ...
  //////////////////////////////////////////////////////////////////////////
  const auto& static_objects() const { return _static_objects; }
  ...
  //
  // Return:  bool	- true = collision happened(reaction happens within the
  // function), false = no collision
  //
  // Parameters: CCollideable* pCol	- the component to check
  //
  // Purpose:  this function checks for collision with a single object(type
  // BSPHERE)
  ...
  //
  // Return:  bool - true = values within fEpsilon of each other, false =
  // they're not
  //
  ...
  // Get/Set delta time
  //////////////////////////////////////////////////////////////////////////
  void SetTime(float fT) { m_fElapsed = fT; }
  float GetTime(void) { return m_fElapsed; }
  ...
  //
  // Purpose:  These functions add a component to the respective
  // list(static/nonstatic/etc)
  ////////////////////////////////////////////////////////////////////////////////
  ...
  //					  bool	isStatic				- whether the object is a static or
  // nonstatic object 					  bool isReactor			- true if the object
  // reacts to
  // collisions 					  unsigned int objType	- the type of object to attach
  // the component to
  //
  // Purpose:  Creates a collideable component with the data passed in
  ////////////////////////////////////////////////////////////////////////////////
  static collider* create_component(scd::object& owner, bool isStatic,
                                    bool isReactor, unsigned int objType);
  ...
  //
  // Purpose:  loops through the components calling TestCollision on certain
  // pairs
  //					ie. all nonstatics to all nonstatics, and all nonstatics to the
  // statics
  ////////////////////////////////////////////////////////////////////////////////
  ...
  //
  // Parameters: CCollideable* obj1, obj2		- the two objects to test collision
  // with
  //
  ...
  // Parameters: void
  //
  // Purpose:  creates the renderable collision volumes for all AABBs after
  // combining
  ////////////////////////////////////////////////////////////////////////////////
  ...
  // Parameters: void
  //
  // Purpose:  calls condense collision boxes a few times to make sure no
  // adjacent
  //					box pairs are left, and then creates the renderable collision
  // volumes 					for them
  ////////////////////////////////////////////////////////////////////////////////
  void CombineLinedBoxes(void) {
    // by our powers combined
    CondenseCollisionBoxes();
    CondenseCollisionBoxes();
    CondenseCollisionBoxes();
  ...
  // Parameters: void
  //
  // Purpose:  sets all the collision components' "checked this frame" status to
  // false
  ////////////////////////////////////////////////////////////////////////////////
  void SetAllNotChecked(void) {
    std::map<unsigned int,
             CCollideable*,
             less<unsigned int>,
             scd::allocator<pair<unsigned int, CCollideable*>>>::iterator
      insiter;
    for (insiter = m_cNonStaticObjects.begin();
         insiter != m_cNonStaticObjects.end();
         ++insiter) {
  ...
  //////////////////////////////////////////////////////////////////////////
  scd::object* GetTurkeyDestroyObject(void) { return m_pTurkeyDestroy; }
  scd::object* GetPieDestroyObject(void) { return m_pPieDestroy; }
  scd::object* GetBananaDestroyObject(void) { return m_pBananaDestroy; }
  ...
  //
  // Parameters: CCollideable* obj1, obj2		- the two objects that will be
  // tested
  //					  scd::vector3* vReflect	- the normal from the collision point
  //					  bool bReact						- if the first object should react to the
  // collision
  //
  // Purpose: These functions take two components for testing collision
  //					the system allows for objects to be used in functions that don't
  // match 					a.k.a. - a sphere and a box using OBB to AABB tests will
  // work the order the shapes are mentioned is the order the objects of the
  // same
  // respective BVTYPE have to be passed in 					vReflect is a vector
  // passed in that is set to the reflect vector or the 					surface normal
  // of the collision 					bReact is a bool for whether or not the object
  // will react to the collision 					with a translation/event/etc
  ////////////////////////////////////////////////////////////////////////////////
  ...
  bool CapsuleToOBBIntersection(CCollideable* obj1, CCollideable* obj2,
                                scd::vector3* tPosition, bool bReact);
  bool SphereToOBBIntersection(
    CCollideable* obj1, CCollideable* obj2, scd::vector3* tPosition,
    bool bReact); // bReact is pretty much always going to be true
  ...
  bool OBBToAABBIntersection(CCollideable* obj1, CCollideable* obj2,
                             scd::vector3* tPosition, bool bReact);
  bool LineToSphereIntersection(TLine tLine, TSphere tSphere,
                                scd::vector3& tColPoint);
  ...
  bool RayToSphereIntersection(TRay ray, TSphere sphere, float& t,
                               scd::vector3& intersect);
  bool RayToTriangleIntersection(TTriangle tri, TRay ray, float& t);
  bool MovingSphereToAABBIntersection(CCollideable* obj1, CCollideable* obj2,
                                      scd::vector3 tVelocity,
                                      scd::vector3* vReflect, bool bReact);
  bool MovingSphereToTriangleIntersection(CCollideable* obj, TTriangle* tri,
                                          scd::vector3* vReflect,
                                          scd::vector3* tCol, float& t,
                                          bool bReact);
  ...
  //
  // Purpose:  gets the normal of the side that the collision point is on on the
  // box
  ////////////////////////////////////////////////////////////////////////////////
  ...
  //
  // Purpose:  returns the normal of the sphere in the direction of the colPt
  // from the center
  ////////////////////////////////////////////////////////////////////////////////
  ...
  ////////////////////////////////////////////////////////////////////////////////
  scd::vector3 GetCloserPt(scd::vector3 tPt1, scd::vector3 tPt2,
                           scd::vector3 tTestpt);
  ...
  //
  // Purpose:  returns the closest point to the given AABB/OBB from the test
  // point
  ////////////////////////////////////////////////////////////////////////////////
  void ClosestPointToAABB(TAABB tBox, scd::vector3 tTestPt,
                          scd::vector3& tClosest);
  void ClosestPointToOBB(TOBB tBox, scd::vector3 tTestPt,
                         scd::vector3& tClosest);
  ...
  ////////////////////////////////////////////////////////////////////////////////
  int HalfSpaceTest(scd::vector3 tNorm, scd::vector3 tPlanePt,
                    scd::vector3 tTest);
  ...
  ////////////////////////////////////////////////////////////////////////////////
  scd::vector3 ClosestPtPointTriangle(scd::vector3 p, scd::vector3 a,
                                      scd::vector3 b, scd::vector3 c);
  ...
  //
  // Purpose: tests an individual object against the wall and performs the
  // appropriate
  ...
  //					  const scd::vector3& start, d	- the start point and direction(d)
  // of the ray 					  float& t	- the time of intersection
  //
  // Purpose: tests whether or not the given ray(start/d) intersects the given
  //					triangle(vert0-1-2,norm)
  ////////////////////////////////////////////////////////////////////////////////
  bool IntersectRayTriangle(const scd::vector3& vert0,
                            const scd::vector3& vert1,
                            const scd::vector3& vert2, const scd::vector3& norm,
                            const scd::vector3& start, const scd::vector3& d,
                            float& t);
};
