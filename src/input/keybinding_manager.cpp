...
// do the same for the pause version of this
	...
	scd::vector4 textColor(1.0f, .6f, .2f, 255);
			...
			for(int i=0; i<256; i++) // 256 is size of chkeys buffer
			{
				if(KEYDOWN(pKeys, i))
				{
					...
					if(i == DIK_ESCAPE)
					{
						...
						pMgr->m_tKeyInfo[pMgr->m_nIndexToChange].pFont->SetColor(scd::vector4(1.0f, .6f, .2f, 1.0f));
...
int CKeybindsManager::SetKeyCallback(lua_State* pLua)
{
	...
	if(!pMgr->m_bCatchingInput && nIndexToChange >= 0 && nIndexToChange < NUM_REBINDABLE_KEYS)
	{
		...
		pMgr->m_tKeyInfo[nIndexToChange].pFont->SetColor(scd::vector4(1.0f, 1.0f, 1.0f, 1.0f));
