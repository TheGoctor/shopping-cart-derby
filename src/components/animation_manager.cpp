...
void CAnimateManager::Init(void)
{
	...
	pEM->RegisterEvent("Shutdown", (scd::base_component*)GetInstance(), ShutdownCallback);
	
	string szEvent = "ShutdownObjects";
	szEvent += (char)GAMEPLAY_STATE;
	pEM->RegisterEvent(szEvent, (scd::base_component*)GetInstance(), DisableCallback);
}
...
int CAnimateManager::CreateAnimComp(lua_State* pLua)
{
	...
	scd::object* pObj = (scd::object*)lua_topointer(pLua, nLastParam++);
...
CAnimateComponent* CAnimateManager::CreateAnimateComp(scd::object* pParent, DXMesh* pMesh)
{
...
// Shutdown
void CAnimateManager::ShutdownCallback(IEvent*, scd::base_component*)
{
...
void CAnimateManager::Shutdown(void)
{
	...
	map<unsigned int, CAnimateComponent*, less<unsigned int>,
		scd::allocator<pair<unsigned int, CAnimateComponent*>>>::iterator compIter
...
void CAnimateManager::DisableCallback(IEvent*, scd::base_component*)
{
...
void CAnimateManager::Disable()
{
	...
	map<unsigned int, CAnimateComponent*, less<unsigned int>, 
		scd::allocator<pair<unsigned int, CAnimateComponent*>>>::iterator pIter;
	CEventManager* pEM = CEventManager::GetInstance();
	...
	while(pIter != m_cAnimateComps.end())
	{
		...
		scd::objectManager::GetInstance()->DestroyObject(pIter->second->GetParent());
		MMDEL(pIter->second);
		++pIter;
	}
