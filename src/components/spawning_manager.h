...
public:
   ...
   *
   * Ins:				IEvent*		-
   *TUpdateStateEvent* scd::base_component*	-
   *
   * Outs:
   *
   * Returns:
   ...
   *
   * Ins:				IEvent*		-
   *TGoalItemCollectedEvent* scd::base_component*	-
   *
   * Outs:
   *
   * Returns:
   ...
   *						Sends "GoalItemCollected" event
   *if the Goal Item isn't collidable.
   *
   * Ins:				IEvent*		-
   *TGoalItemCollectedEvent* scd::base_component*	-
   *
  ...
  inline unsigned GetGoalItem(int nIndex) { return m_nGoalItems[nIndex]; }
...
private:
  ...
  scd::map<unsigned int, CGoalItems*> m_cGoalItems;

  // Each Held Item that is in the level
  scd::map<unsigned int, CHeldItemComponent*> m_cHeldItems;

  // List of each department in the level
  scd::map<EDepartment, CDepartment*> m_cDepartments;

  // List of all the locations that Held Items can spawn
  scd::list<TSpawnLocation> m_cHeldItemLocations;

  std::string m_szGoalItemNames[MAX_GOAL_ITEMS];
  ...
  /**
   * @brief Spawns a goal item in a random department.
   */
  void SpawnGoalItem();
