...
void CLevelManager::Init()
{
	...
	// Get Singleton
	scd::base_component* pInstance = (scd::base_component*)GetInstance();
	...
	//scd::object* pMaxXWall = scd::objectManager::GetInstance()->CreateObject(szWallName1);
	//std::string szWallName2 = "MinXWall";
	//scd::object* pMinXWall = scd::objectManager::GetInstance()->CreateObject(szWallName2);
	//std::string szWallName3 = "MaxZWall";
	//scd::object* pMaxZWall = scd::objectManager::GetInstance()->CreateObject(szWallName3);
	//std::string szWallName4 = "MinZWall";
	//scd::object* pMinZWall = scd::objectManager::GetInstance()->CreateObject(szWallName4);

	//TPlane MaxX, MinX, MaxZ, MinZ;
	//MaxX.cPlanePoint = scd::vector3(48.0f, 0.0f, 0.0f);
	//MaxX.cPlaneNormal = scd::vector3(-1.0f, 0.0f, 0.0f);

	//MinX.cPlanePoint = scd::vector3(-48.0f, 0.0f, 0.0f);
	//MinX.cPlaneNormal = scd::vector3(1.0f, 0.0f, 0.0f);

	//MaxZ.cPlanePoint = scd::vector3(0.0f, 0.0f, 36.0f);
	//MaxZ.cPlaneNormal = scd::vector3(0.0f, 0.0f, -1.0f);

	//MinZ.cPlanePoint = scd::vector3(0.0f, 0.0f, -36.0f);
	//MinZ.cPlaneNormal = scd::vector3(0.0f, 0.0f, 1.0f);
...
// Change State
void CLevelManager::Level1SelectedCallback(IEvent*, scd::base_component*)
{
	...
	// Change States
	SendStateEvent("StateChange", (scd::base_component*)pLM,
...
void CLevelManager::Shutdown(IEvent*, scd::base_component*)
{
	CLevelManager* pLM = CLevelManager::GetInstance();
	list<CRenderComponent*, scd::allocator<CRenderComponent*>>::iterator pLIter;
	...
	list<CRenderComponent*, scd::allocator<CRenderComponent*>>::iterator pCIter;
...
void CLevelManager::BuildLevel(char* szNodeMap)
{
	...
	if(fin.is_open())
	{
		...
		for(int i = 0; i < m_objectcount; ++i)
		{
			...
			// worldMat
			fin.read((char*)&m_levelNodes[i].tWorldMatrix, sizeof(scd::transform));
		}
	...
	scd::vector3 vWorldSpacePos;
	scd::vector3 vRotateAxis;
	ModelManager* pMM = ModelManager::GetInstance();
	scd::objectManager* pOM = scd::objectManager::GetInstance();
	Renderer* pRenMan = Renderer::GetInstance(); 
	CRenderComponent* pRC;
	scd::object *pObj, *pColObj;
	...
	for(int i = 0; i < m_objectcount; ++i)
	{
		...
		scd::transform rot(m_levelNodes[i].tWorldMatrix);
		rot._41 = 0.0f;
		rot._42 = 0.0f;
		rot._43 = 0.0f;
		rot._44 = 0.0f;
		...
		if(tolower(m_levelNodes[i].szName[0]) == 'p')
		{
			...
			for(unsigned int inner = 0; inner < strlen; ++inner)
			{
				...
				if(isdigit(m_levelNodes[i].szName[inner]))
				{
					...
					scd::object* playPtr = pOM->GetObjectByName(player);
					// move player
					if(!playPtr)
						continue;
...
void CLevelManager::BuildShadows(char* szNodeMap)
{
	...
	if(fin.is_open())
	{
		...
		for(int i = 0; i < nShadowObjectCount; ++i)
		{
			...
			// worldMat
			fin.read((char*)&m_shadowNodes[i].tWorldMatrix, sizeof(scd::transform));
		}
	...
	for(int i = 0; i < nShadowObjectCount; ++i)
	{
		...
		scd::transform rot(m_shadowNodes[i].tWorldMatrix);
		rot._41 = 0.0f;
		rot._42 = 0.0f;
		rot._43 = 0.0f;
		rot._44 = 0.0f;
...
void CLevelManager::FindAndCreateShadow(scd::object* pParentObj, ERenderContext eObjRenContext)
{
	//// Get Singletons
	//ModelManager* pMM = ModelManager::GetInstance();
	//scd::objectManager* pOM = scd::objectManager::GetInstance();
	...
	//	//pShadowObj->GetTransform()->TranslateFrame(scd::vector3(0.0f, 0.1f, 0.0f));
...
void CLevelManager::Update(IEvent*, scd::base_component*)
{
	...
	{
		// Render the Geometry
		list<CRenderComponent*, scd::allocator<CRenderComponent*>>::iterator pcGeoIter;
		...
		while(pcGeoIter != CLevelManager::GetInstance()->m_cLevelRenderComps.end())
		{
			...
			SendRenderEvent("AddToSet", (scd::base_component*)GetInstance(), pRC->GetParent(), PRIORITY_IMMEDIATE);
	...
	if(CLevelManager::GetInstance()->GetDrawCollision())
	{
		...
		list<CRenderComponent*, scd::allocator<CRenderComponent*>>::iterator pcColIter;
		...
		while(pcColIter != CLevelManager::GetInstance()->m_cLevelCollisionRenderComps.end())
		{
			...
			SendRenderEvent("AddToSet", (scd::base_component*)GetInstance(), pCRC->GetParent(), PRIORITY_IMMEDIATE);
...
void CLevelManager::LoadLocations()
{
	...
	for(int nIndex = 0; nIndex < CLevelManager::GetInstance()->GetNumNodes(); ++nIndex)
	{
		...
		{	
			...
			if(_stricmp(szTokenizer, "CheckoutCollision") == 0)
			{
				...
				SendIEvent("InitCheckoutLocation", (scd::base_component*)GetInstance(), NULL, PRIORITY_NORMAL);
	...
	scd::object* pCheckout = scd::objectManager::GetInstance()->CreateObject(szCheckout);
	//scd::object* pCheckout = MMNEW(scd::object) scd::object(szCheckout);
	...
	TAABB pCHAABB;
	pCHAABB.cBoxMax = scd::vector3(0.0f, 0.0f, 0.0f);
	pCHAABB.cBoxMin = scd::vector3(0.0f, 0.0f, 0.0f);
	...
	{
		scd::vector3 curVert = pOriginalVerts->m_vVertices[i];
	...
	scd::vector3 vNewNodePos = (pCHAABB.cBoxMax + pCHAABB.cBoxMin)*.5f; // average the points to get the center
	...
	scd::object* pNonOrientedPallets = scd::objectManager::GetInstance()->CreateObject("nonOrientPal");
	TSphere sphur;
	sphur.cPosition = scd::vector3(0.0f, 0.0f, 0.0f);
	sphur.fRadius = 1.5f;
	...
	pNOPal->GetParent()->GetTransform()->TranslateFrame(scd::vector3(-50.5, 0, 44));

	scd::object* pNonOrientedPallets2 = scd::objectManager::GetInstance()->CreateObject("nonOrientPal2");
	TSphere sphur2;
	sphur2.cPosition = scd::vector3(0.0f, 0.0f, 0.0f);
	sphur2.fRadius = 1.5f;
	...
	pNOPal2->GetParent()->GetTransform()->TranslateFrame(scd::vector3(46, 0, 47));
}