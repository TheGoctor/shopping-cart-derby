...
void CSpawningManager::Init()
{
	...
	pEM->RegisterEvent("GoalItemDespawned", (scd::base_component*)GetInstance(), Despawned);
	pEM->RegisterEvent("GoalItemCollected", (scd::base_component*)GetInstance(), 
		GoalItemCollected);
	pEM->RegisterEvent("DestroyObject", (scd::base_component*)GetInstance(), 
		DestroyObject);
	pEM->RegisterEvent("GoalItemCollision", (scd::base_component*)GetInstance(), 
		GoalItemCollision);
	pEM->RegisterEvent("OverlayUnshow", (scd::base_component*)GetInstance(), 
		HandleStartUpdate);
	
	szEvent = "InitObjects";
	szEvent += GAMEPLAY_STATE;
	pEM->RegisterEvent(szEvent, (scd::base_component*)GetInstance(), HandleInitObjects);
...
void CSpawningManager::Shutdown(IEvent*, scd::base_component*)
{
	...
	map<EDepartment, CDepartment*, less<unsigned int>, scd::allocator
	...
	map<unsigned int, CGoalItems*, less<unsigned int>, scd::allocator
	...
	map<unsigned int, CHeldItemComponent*, less<unsigned int>, scd::allocator
	...
	for(int nIndex = 0; nIndex < objectcount; ++nIndex)
	{
		...
		{	
			for(unsigned nDepartment = 0; nDepartment < MAX_DEPARTMENTS; ++nDepartment)
			{
				...
				{
					map<EDepartment, CDepartment*, less<unsigned int>,
						scd::allocator<pair<EDepartment, CDepartment*>>>::iterator DepartmentIter =
...
CHeldItemComponent* CSpawningManager::CreateHeldItemComponent(scd::object* pObj)
{
...
EHeldItemType CSpawningManager::GetHeldItemType(scd::object* pcObject)
{
	map<unsigned int, CHeldItemComponent*, less<unsigned int>, 
		scd::allocator<pair<unsigned int, CHeldItemComponent*>>>::iterator ItemIter;
...
void CSpawningManager::GoalItemCollision(IEvent* iEvent, scd::base_component*)
{
	...
	// Tell everyone else
	SendGoalItemCollectedEvent("GoalItemCollected", (scd::base_component*)GetInstance(),
