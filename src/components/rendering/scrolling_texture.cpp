...
void CScrollingUVComp::UpdateStoryTimeCart(float fDT) {
  ...
  for (unsigned int uVert = 0; uVert < m_pMesh->GetVertCount(); ++uVert) {
    if (pVertInfo->m_vUV[uVert].y >= -0.5f) {
      pVertInfo->m_vUV[uVert].x +=
          fDT * (STORY_TIME_CART_SCROLL_RATE * m_fSpeed);
    }
  }
...
void CScrollingUVComp::AccelerateCallback(IEvent* pEvent, IComponent* pComp) {
  ...
  if (CScrollingUVComp::CheckForPlayerMatch(
          pSUVC->GetParent(), pData->m_pPlayer)) {
...
// Check for Player
bool CScrollingUVComp::CheckForPlayerMatch(
    CObject* pParentObj,
    CObject* pOtherObj) {
  // Get Object Names
...
// Destroy Object
void CScrollingUVComp::DestroyObjectCallback(
    IEvent* pEvent,
    IComponent* pComp) {
  // Get Component from Event
  CScrollingUVComp* pSUVC = (CScrollingUVComp*)pComp;
