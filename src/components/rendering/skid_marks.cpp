...
// Mesh
CSkidMarkMesh::CSkidMarkMesh(void)
    : DXMesh()
    , m_pRenComp(NULL)
    , m_bActive(false)
    , m_fWidth(0.1f)
...
void CSkidMarks::Init(void) {
  ...
  m_pLeftSkidEmitter->GetTransform()->TranslateFrame(
      FindLeftEmitterPos(m_eCharacterSetType));
  ...
  m_pRightSkidEmitter->GetTransform()->TranslateFrame(
      FindRightEmitterPos(m_eCharacterSetType));
  ...
  if (m_eCharacterSetType == CS_BIKERS) {
    ...
    m_pLDustEmitter->GetTransform()->TranslateFrame(
        D3DXVECTOR3(0.4f, 1.0f, -1.0f));
    ...
    m_pRDustEmitter->GetTransform()->TranslateFrame(
        D3DXVECTOR3(-0.4f, 1.0f, -1.0f));
...
void CSkidMarks::CreateNewPair(void) {
  ...
  } else // Create a New Pair
  {
    ...
    pSMP->SetFrames(
        m_pLeftSkidEmitter->GetTransform(),
        m_pRightSkidEmitter->GetTransform());
...
void CSkidMeshPair::CreateMeshs(ERenderContext eRC) {
  ...
  CObject* pSkidMeshObjLeft =
      CObjectManager::GetInstance()->CreateObject(szObjName);
  szObjName = "RightSkidMesh";
  szObjName += (char)nSkidCounter;
  CObject* pSkidMeshObjRight =
      CObjectManager::GetInstance()->CreateObject(szObjName);
  ...
  // Create Render Comp
  CRenderComponent* pSkidRenCompLeft =
      Renderer::GetInstance()->CreateRenderComp(
          pSkidMeshObjLeft, (DXMesh*)&m_cLeftMesh, eRC, RF_VERT_TEX2);
  CRenderComponent* pSkidRenCompRight =
      Renderer::GetInstance()->CreateRenderComp(
          pSkidMeshObjRight, (DXMesh*)&m_cRightMesh, eRC, RF_VERT_TEX2);
...
void CSkidMarks::Update(float fDT) {
  ...
  if (m_bAccelerating) {
    if (m_pLDustEffect) {
      m_pLDustEffect->SwitchOnOffEmitters(
          EC_TYPE_CART_SMOKE, true); // 0.001F * fDT);
      m_pLDustEffect->SetLifespan(EC_TYPE_CART_SMOKE, 0.001F * fDT);
    }
    if (m_pRDustEffect) {
      m_pRDustEffect->SwitchOnOffEmitters(
          EC_TYPE_CART_SMOKE,
          true); // SetLifespan(EC_TYPE_CART_SMOKE, 0.001F * fDT);
...
void CSkidMarks::Drift(TInputEvent* pcObjEvent) {
  ...
  if (CheckForPlayerMatch(
          m_pParentObj,
          pcObjEvent->m_pPlayer)) // pcObjEvent->m_pPlayer == GetParent())
  {
    m_bDrift = true;
