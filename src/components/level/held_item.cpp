...
void CHeldItemComponent::Init() {
  ...
  CEventManager::GetInstance()->RegisterEvent(
    "HeldItemCollision", this, HeldItemCollision);
  CEventManager::GetInstance()->RegisterEvent(
    "HeldItemCollected", this, HeldItemCollected);
}
...
void CHeldItemComponent::Update(IEvent* piEvent,
  ...
  if (pcHeldItem->m_bSpawned) {
    ...
    pcHeldItem->m_pParent->GetTransform()->RotateFrame(scd::vector3(0, 1, 0),
                                                       fDT * 1.5f);
...
void CHeldItemComponent::HeldItemCollision(IEvent* iEvent,
  ...
  if (pHeldItem == pComp->m_pParent) {
    ...
    if (pComp->m_tSpawnLocation && pComp->m_tSpawnLocation->m_cEndCap) {
      SendObjectEvent(
        "BlueLightSpecialDespawned", pComp, pComp->m_tSpawnLocation->m_cEndCap);
    }
    // Tell everyone else
...
void CHeldItemComponent::HeldItemCollected(IEvent* iEvent,
                                           scd::base_component* comp) {
  THeldItemCollected* tEvent = (THeldItemCollected*)iEvent->GetData();
  CHeldItemComponent* pComp  = (CHeldItemComponent*)comp;
...
void CHeldItemComponent::PauseUpdateCallback(IEvent*,
