 ...
 * Purpose:   		This will represent the Goal Item in the world.
                                        When created, Init should be called and
 a EGoalItemType should be given. Goal Items will register for an update event
 and will despawn after a set period of time. This time is set in the init
 function.
 ******************************************************************************/
#include "goal_item.h"
...
CGoalItems::CGoalItems(scd::object* pObj)
    : m_pObject(pObj) {
  ...
  szEvent += GAMEPLAY_STATE;
  event_manager.register_event(szEvent, this, ShutdownGoalItem);
}
...
void CGoalItems::Update(IEvent* piEvent, scd::base_component* piComponent) {
  // Get component
  ...
  pcGoalItem->m_pObject->GetTransform()->GetLocalMatrix()._42 =
      sin(pcGoalItem->m_fDespawnTimer * 2.5f) * .2f + 1.3f;
  pcGoalItem->m_pObject->GetTransform()->RotateFrame(
  ...
  if (pcGoalItem->m_bCollidable == true) {
    if (pcGoalItem->m_fDespawnTimer <= 5.0f) {
      ...
      SendGoalItemEvent(
          "GoalItemBlinking", pcGoalItem, pcGoalItem->m_eType, pcGoalItem);
    }

    // AddToSet
    if (pcGoalItem->m_bBlink == false) {
      SendRenderEvent(
          "AddToSet", pcGoalItem, pcGoalItem->GetParent(), PRIORITY_IMMEDIATE);
    }
  }
...
void CGoalItems::Despawn() { m_bCollidable = false; }

void CGoalItems::ShutdownGoalItem(
    IEvent* /*piEvent*/,
    scd::base_component* piComponent) {
  // Get component
  CGoalItems* pcGoalItem = (CGoalItems*)piComponent;

  pcGoalItem->m_fDespawnTimer =
      0.0f; // so we catch the despawn in update next update
}

void CGoalItems::PauseUpdateCallback(IEvent*, scd::base_component* pComp) {
  // Get the Effect Comp
