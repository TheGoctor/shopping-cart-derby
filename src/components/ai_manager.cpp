...
#include "Components\AI\CAscd::base_component.h"
...
int CAIManager::SetGoalTri(lua_State* pLua)
{
	...
	map<unsigned int, CAscd::base_component*, less<unsigned int>,
		scd::allocator<pair<unsigned int, CAscd::base_component*>>>::iterator pIter = 
		GetInstance()->m_cAscd::base_components.find(nID);

	if(pIter == GetInstance()->m_cAscd::base_components.end())
	{
	...
	if(nTri != -1)
	{
		map<int, TTri*, less<int>,
			scd::allocator<pair<int, TTri*>>>::iterator pTriIter =
...
void CAIManager::ToggleInfo(IEvent*, scd::base_component*)
{
...
{
	m_bShowInfo = false;
	SendStringEvent("LoadLevel", (scd::base_component*)GetInstance(), "");

	string szEvent = "Update";
	szEvent += GAMEPLAY_STATE;
	CEventManager* pEM = CEventManager::GetInstance();
	pEM->RegisterEvent(szEvent, (scd::base_component*)GetInstance(), Update);
	pEM->RegisterEvent("GoalItemInit", (scd::base_component*)GetInstance(), GoalItemInit);
	pEM->RegisterEvent("PlayerCreated", (scd::base_component*)GetInstance(), SetupOpponents);
	pEM->RegisterEvent("ToggleInfo", (scd::base_component*)GetInstance(), ToggleInfo);
	pEM->RegisterEvent("LoadLevel", (scd::base_component*)GetInstance(), LoadLevel);
	pEM->RegisterEvent("Shutdown", (scd::base_component*)GetInstance(), Shutdown);
}

void CAIManager::LoadLevel(IEvent* pEvent, scd::base_component*)
{
...
void CAIManager::SetupOpponents(IEvent* piEvent, scd::base_component*)
{
	// NOTE: This must be done AFTER the CAscd::base_component is created
	...
	std::map<unsigned int, CAscd::base_component*, less<unsigned int>,
		scd::allocator<pair<unsigned int, CAscd::base_component*>>>::iterator pCompAIIter;
	pCompAIIter = GetInstance()->m_cAscd::base_components.begin();

	while(pCompAIIter != GetInstance()->m_cAscd::base_components.end())
	{
...
void CAIManager::GoalItemInit(IEvent* piEvent, scd::base_component*)
{
...
void CAIManager::Update(IEvent*, scd::base_component*)
{
}
...
void CAIManager::LoadNavMesh(string szFileName)
{
	...
	if(fin.is_open())
	{
		...
		for(unsigned int i = 0; i < num; ++i)
		{
			...
			verts[i].normal = scd::vector3(0.0f, 1.0f, 0.0f);
			verts[i].uv.x = 0.0f;
			verts[i].uv.y = 0.0f;
		}
...
void CAIManager::CreateConnections(string szFileName)
{
	...
	{
		map<int, TTri*, less<int>,
			scd::allocator<pair<int, TTri*>>>::iterator pTriIter = m_cTris.begin();
	...
	{
		map<int, TTri*, less<int>,
			scd::allocator<pair<int, TTri*>>>::iterator pStart = m_cTris.begin();

		map<int, TTri*, less<int>,
			scd::allocator<pair<int, TTri*>>>::iterator pEnd;
		...
		if(out.is_open())
		{
			...
			{
				map<int, TTri*, less<int>,
					scd::allocator<pair<int, TTri*>>>::iterator pConnectionIter = 
...
int CAIManager::CreateAscd::base_component(lua_State* pLua)
{
	scd::object* pObj = (scd::object*)lua_topointer(pLua, -2);
	bool bRandomItems = (lua_toboolean(pLua, -1) != 0);
	lua_pop(pLua, 2);
	CreateAscd::base_component(pObj, bRandomItems);
	
	return 0;
}

CAscd::base_component* CAIManager::CreateAscd::base_component(scd::object* pObj, bool bRandomItems)
{
	CAscd::base_component* comp = MMNEW(CAscd::base_component(pObj));
	...
	GetInstance()->m_cAscd::base_components.insert(make_pair((unsigned int)
		pObj->GetID(), comp));
...
// Events Start
void CAIManager::Shutdown(IEvent*, scd::base_component*)
{
	...
	while(pAIManager->m_cAscd::base_components.empty() == false)
	{
		MMDEL(pAIManager->m_cAscd::base_components.begin()->second);

		pAIManager->m_cAscd::base_components.erase(
			pAIManager->m_cAscd::base_components.begin());
	}
...
int CAIManager::Pathplan(const int nStart, const int nEnd)
{
	...
	while(m_cOpen.empty() == false)
	{
		...
		map<TTri*, TPlannerNode*, less<TTri*>,
			scd::allocator<pair<TTri*, TPlannerNode*>>>::iterator pCreatedIter;
	...
	map<int, TTri*, less<int>,
		scd::allocator<pair<int, TTri*>>>::iterator pIter = m_cTris.begin();
	...
	// Clean up memory
	map<TTri*, TPlannerNode*, less<TTri*>,
		scd::allocator<pair<TTri*, TPlannerNode*>>>::iterator pDelIter = 
		m_cCreated.begin();
...
string& CAIManager::GetAIInfo()
{
	if(m_bShowInfo == true)
	{
		map<unsigned int, CAscd::base_component*, less<unsigned int>,
			scd::allocator<pair<unsigned int, CAscd::base_component*>>>::iterator pIter =
			m_cAscd::base_components.begin();

		m_szInfo = "AI:  | ";

		while(pIter != m_cAscd::base_components.end())
		{
