...
// Constructor
CInventoryComponent::CInventoryComponent(scd::object* pParent, int nPlayer)
...
// Collect Item
void CInventoryComponent::GoalItemCollectedCallback(IEvent* e, scd::base_component* comp)
{
	...
	int eType = CSpawningManager::GetInstance()->GetGoalItemType(pcObjEvent->
		m_pcGoalItem);
	scd::object* pCollector = pcObjEvent->m_pcCollector;
...
void CInventoryComponent::PickupItemCollectedCallback(IEvent* e, scd::base_component* comp)
{
	// Get the Values from the Event
...
void CInventoryComponent::HeldItemCollected(IEvent* iEvent, scd::base_component* pComponent)
{
...
void CInventoryComponent::UseHeldItem1(IEvent* pEvent, scd::base_component* pComponent)
{
...
void CInventoryComponent::UseHeldItem2(IEvent* pEvent, scd::base_component* pComponent)
{
	TInputEvent* pInputEvent = (TInputEvent*)pEvent->GetData();
	CInventoryComponent* pComp = (CInventoryComponent*)pComponent;
...
// Rammed
void CInventoryComponent::Rammed(IEvent* e, scd::base_component* comp)
{
	// This should be moved to the inventorycomponent instead of the manager, no?
	TRamEvent* pEvent = (TRamEvent*)e->GetData();
	CInventoryComponent* pComp = (CInventoryComponent*)comp;

	scd::object* pRammerObject = pEvent->m_pcRammer;
	scd::object* pRammedObject = pEvent->m_pcRammee;
	...
	if(pComp->m_pParent == pRammedObject)
	{
		...
		for(int item = 0; item < NUM_ITEMS_IN_LEVEL; ++item)
		{
			...
			if(item == nItemToRemove && pComp->m_bGoalItemCollectionState[item])
			{
				...
				scd::vector3 vObjPos =  pComp->m_pParent->GetTransform()->GetWorldPosition();
...
void CInventoryComponent::checkout(IEvent* e, scd::base_component* comp)
{
	CInventoryComponent* pComp = (CInventoryComponent*)comp;
...
void CInventoryComponent::HandleInit(IEvent*, scd::base_component* comp)
{
	CInventoryComponent* pComp = (CInventoryComponent*)comp;
...
// Collect Item
void CInventoryComponent::GoalItemInitCallback(IEvent* e, scd::base_component* comp)
{
	// Get the Values from the Event
...
void CInventoryComponent::StunCallback(IEvent* e, scd::base_component* comp)
{
...
void CInventoryComponent::HeldItemSteal(IEvent* e, scd::base_component* comp)
{
	// Get the Values from the Event
	...
	if(pComp->m_pParent == tEvent->m_pcCollider)
	{
		...
		scd::object* pHumanObj = scd::objectManager::GetInstance()->GetObjectByName("Player0");
...
void CInventoryComponent::PlayCollectionSound(scd::object *player)
{
...
void CInventoryComponent::PlayAllItemCollectionSound(scd::object *player)
{
...
void CInventoryComponent::GoalItemSpawnedCallback(IEvent* pEvent, scd::base_component* iComp)
{
