...
slider::slider(scd::object* pObj)
  : button(pObj)
  , m_fSliderValue(0.0f)
  , m_fStateEnteredSliderValue(69.0f) {}

// Lua stuff

// Lua Prototype: CreateSliderComponent(Object, "EventName",
// "ButtonTexture.png", nBarStartX, nBarStartY, nBarEndX, nBarEndY,
//										nTextureDepth, bStartSelected, nActiveState)
int CSliderComponent::CreateSliderComponent(lua_State* pLua) {
  // top down is in order in param list. Start at bottom with -1 and work your
  // way to the more negative numbers
  scd::object* pObj      = (scd::object*)lua_topointer(pLua, -11);
  ...
  slider* pSlider = CreateSliderComponent(pObj,
                                                    szEventName,
                                                    szButtonTexName,
                                                    nStartX,
                                                    nStartY,
...
CSliderComponent* CSliderComponent::CreateSliderComponent(
  scd::object* pObj, string szEventName, string szSpriteTextureName,
...
void CSliderComponent::Init(string textureName, int nTextureDepth) {
  ...
  int nTexID0 = CTextureManager::GetInstance()->LoadTexture(szButtonTex.c_str(),
                                                            RGB(255, 255, 255));

  int nTexID1 = CTextureManager::GetInstance()->LoadTexture(
    "Resource\\HUD\\T_Pause_Icon_Glow_D.png", RGB(255, 255, 255));

  // Create the Object
  string szDispObjName = "ButtonDisplayObject";
  szDispObjName += (char)this;
  m_pDisplayObjectButton
    = scd::objectManager::GetInstance()->CreateObject(szDispObjName);
  szDispObjName = "ButtonSelectionDisplayObject";
  szDispObjName += (char)this;

  m_pDisplayObjectButton
    = scd::objectManager::GetInstance()->CreateObject(szDispObjName);
  m_pDisplayObjectSelection
    = scd::objectManager::GetInstance()->CreateObject(szDispObjName);
  ...
  if (m_tSpriteDataButton.m_nTexture != -1) {
    m_pDisplayComponentButton
      = CTextureManager::GetInstance()->CreateSpriteComp(
        m_pDisplayObjectButton,
        m_tSpriteDataButton,
        false); // dont show at start. wait for proper state enter
  }
  if (m_tSpriteDataSelection.m_nTexture != -1) {
    m_pDisplayComponentSelection
      = CTextureManager::GetInstance()->CreateSpriteComp(
        m_pDisplayObjectSelection, m_tSpriteDataSelection, false);
  }
}

void CSliderComponent::SetScreenPosition(int nPosX, int nPosY, int nEndX,
                                         int nEndY) {
...
void CSliderComponent::SelectPressed(IEvent* /*cEvent*/, scd::base_component* cCenter) {
  ...
  if (comp->m_bSelected
    ...
    // Sound Hack
    CWwiseSoundManager::GetInstance()->PlayTheSound(MENU_INVALID_SELECTION,
                                                    GLOBAL_ID);
  }
}

void CSliderComponent::SlideRight(IEvent* /*cEvent*/, scd::base_component* cCenter) {
  CSliderComponent* comp = (CSliderComponent*)cCenter;
  if (comp->m_bSelected
      && comp->m_fTimeSinceSelectionEntry > comp->m_fInputCooldown) {
    ...
    // if the sound cd is up and we wanna play a ticking sound on this slider
    // (aka slider = 2, which is sfx slider)
...
void CSliderComponent::SlideLeft(IEvent* /*cEvent*/, scd::base_component* cCenter) {
  CSliderComponent* comp = (CSliderComponent*)cCenter;
  if (comp->m_bSelected
      && comp->m_fTimeSinceSelectionEntry > comp->m_fInputCooldown) {
...
void CSliderComponent::CalculateAndSendFloatEvent() {
  ...
  scd::vector3 vVolumeVec = scd::vector3((float)m_tSpriteDataButton.m_nX,
                                         (float)m_tSpriteDataButton.m_nY,
                                         0.0f)
                            - m_vStartPosition;
  // from start to end (Entire bar)
  scd::vector3 vSliderBarVec = m_vEndPosition - m_vStartPosition;

  float fVolumeMag    = D3DXVec3LengthSq(&vVolumeVec);
  float fSliderBarMag = D3DXVec3LengthSq(&vSliderBarVec);

  float fVal = max(min(fVolumeMag / fSliderBarMag, 1.0f),
                   0.0f); // current/total 0-1 clamped between 0 and 1
                          // (min(vals, 1) puts us from -inf to 1
                          //			and max(newvals, 0) puts us 0 to inf, so the
                          //overlap is 0 to 1
...
void CSliderComponent::MoveSliderOneTick(int nDirection) {
  ...
  scd::vector3 vSliderBarDirection
    = m_vEndPosition - m_vStartPosition; // from start to end
  float fSliderBarMag
    = D3DXVec3Length(&vSliderBarDirection); // length of whole slider bar
  D3DXVec3Normalize(
    &vSliderBarDirection,
    &vSliderBarDirection); // norm so we can do stuff with it with scaling

  // calculate the "tick" of the movement
  float fAmountToMoveBy
    = (fSliderBarMag / 100.0f)
      * (float)
          nDirection; // 1/100 of the size of the bar in the direction specified
  D3DXVec3Scale(&vSliderBarDirection, &vSliderBarDirection, fAmountToMoveBy);

  // if we are outside the bounds
  scd::vector3 vSliderToStart
    = m_vStartPosition
      - scd::vector3(m_tSpriteDataButton.m_nX + vSliderBarDirection.x,
                     m_tSpriteDataButton.m_nY + vSliderBarDirection.y,
                     0.0f);
  scd::vector3 vSliderToEnd
    = m_vEndPosition
      - scd::vector3(m_tSpriteDataButton.m_nX + vSliderBarDirection.x,
                     m_tSpriteDataButton.m_nY + vSliderBarDirection.y,
                     0.0f);

  // if the angle between them is less than 90 (dot is positive), we know the
  // slider is no longer between them
  ...
  // apply that movement to the selected picture slider (so they match up on
  // location)
...
void CSliderComponent::SetSliderToValue(float fAmount, bool fSendValue) {
  /* // This isn't necessary anymroe but keeping just in case
    // Get comparison values
  scd::vector3 vSliderBarDirection = m_vEndPosition - m_vStartPosition; // from
  start to end float fSliderBarMag = D3DXVec3Length(&vSliderBarDirection); //
  length of whole slider bar D3DXVec3Normalize(&vSliderBarDirection,
  &vSliderBarDirection); // norm so we can do stuff with it with scaling

  float fVal = (fSliderBarMag*fAmount) / 100.0f; // convert from 0-100 space to
  start-finish space

  D3DXVec3Scale(&vSliderBarDirection, &vSliderBarDirection, fVal); // offset
  from start the amount we want is scd::vector3 vNewPos = vSliderBarDirection +
  m_vStartPosition;

  // if we are outside the bounds
  scd::vector3 vSliderToStart = m_vStartPosition - vNewPos;
  scd::vector3 vSliderToEnd = m_vEndPosition - vNewPos;

  // if the angle between them is less than 90 (dot is positive), we know the
  slider is no longer between them if(D3DXVec3Dot(&vSliderToStart,
  &vSliderToEnd) > 0.0f)
  {
    // and thus we gtfo
    return;
  ...
  // apply that movement to the selected picture slider (so they match up on
  location) m_tSpriteDataSelection.m_nX = (int)vNewPos.x;
  ...
  if (m_fSliderValue > fAmount) {
    ...
    while (m_fSliderValue >= fAmount) {
      ...
      if (nNumHit > 300) {
        // CWwiseSoundManager::GetInstance()->PlayTheSound(SASHA_ALLGOALITEMS,
        // GLOBAL_ID);
        break;
      }
    }
...
void CSliderComponent::SliderStateInit(IEvent* /*cEvent*/,
  ...
  case 1:
    pSliderComp->SetSliderToValue(
      CWwiseSoundManager::GetInstance()->GetMusicVolume(), false);
    // cout << CWwiseSoundManager::GetInstance()->GetMusicVolume() << "\t1" <<
    // endl;
    break;
  case 2:
    pSliderComp->SetSliderToValue(
      CWwiseSoundManager::GetInstance()->GetSoundVolume(), false);
    // cout << CWwiseSoundManager::GetInstance()->GetSoundVolume() << "\t2" <<
    // endl;
    break;
  case 3:
    pSliderComp->SetSliderToValue(Direct3DManager::GetInstance()->GetGamma(),
                                  false);
    // cout << Direct3DManager::GetInstance()->GetGamma() << "\t3" << endl;
    break;
  };

  cout << "From " << pSliderComp->m_fStateEnteredSliderValue << " to "
       << pSliderComp->m_fSliderValue << endl;
  pSliderComp->m_fStateEnteredSliderValue = pSliderComp->m_fSliderValue;

  comp->ReInitValues();
}

void CSliderComponent::SliderCancelMain(IEvent* /*cEvent*/,
                                        scd::base_component* cCenter) {
  CSliderComponent* pSliderComp = (CSliderComponent*)cCenter;
...
void CSliderComponent::SliderCancelPause(IEvent* /*cEvent*/,
                                         scd::base_component* cCenter) {
  CSliderComponent* pSliderComp = (CSliderComponent*)cCenter;
...
void CSliderComponent::SliderCancelNonStatic(int eState) {
  if (m_eAssociatedState == eState) {
    ...
    SendFloatEvent(
      m_szSelectionFunc, this, m_fStateEnteredSliderValue, PRIORITY_NORMAL);
  }
}
